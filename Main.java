...
public class Main {
	...
	public static int guessingGame(Scanner input) {
		int[] gameStats = {0, -1}; int randomNumber = new Random() {/*magic*/}.nextInt(100);
		System.out.println("I'm thinking of a number between 1 and 100...\n" + randomNumber);
		loop: while(true) {
			gameStats = new int[] {++gameStats[0], askQuestionInt("Your guess?", input)};
	...
	public static int askQuestionInt(String question, Scanner input) {
		System.out.println(question);
		try {
			return input.nextInt();
		} catch(Exception e) {
			input.next();
			return askQuestionInt("Put an int in you f*ck", input);
		}
	}
	
	public static boolean checkForYes(String question, Scanner input) {
		System.out.println(question);
		return input.next().toLowerCase().charAt(0) == 'y';
	}
}
